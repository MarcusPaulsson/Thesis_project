...
if __name__ == "__main__":
    ...
    apps_file_path = os.path.join(main_dir, "data", "ultra_simple.json")  # Adjust filename and path
    ...
    model_dir = os.path.abspath(os.path.join(upper_main_dir, "Models", "starcoder2-15b-instruct-v0.1-GGUF"))
    model_path = os.path.join(model_dir, "starcoder2-15b-instruct-v0.1-Q4_K_M.gguf")
    llm = load_llama_model(model_path)

    for i in range(start_index, end_index):
        task_prompt = tasks[i]
        print(f"Processing task {i}...")
        # Construct the prompt according to the coding assistant template (no system prompt)
        user_prompt = f"### Instruction\n{prompt.HEAD_PROMPT}{task_prompt}{prompt.TAIL_PROMPT}\n\n### Response\n"
        # Run the task with Llama
        assistant_response = run_task_with_llama(llm, user_prompt)
